# taken from https://github.com/sodiboo/niri-flake/blob/97440574d71b235b601013edf4749397b1ce99a6/flake.nix
# modified to add custom `user-db`s per profile
{
  config,
  lib,
  pkgs,
    ...
    then
      ...
      pkgs.writeTextDir "etc/dconf/profile/${name}" (
        lib.concatMapStrings (x: "${x}\n") (
          (map (db: "user-db:${db}") value.userDbs)
          ++ (map (
              value: let
                db =
...
in {
  disabledModules = ["programs/dconf.nix"];
